...
full_jumps_data<-final_data %>% 
  ...
  select(seas_id:player,age:experience,g_diff:been_allstar_before)
```
...
full_data<-final_data %>% select(seas_id:player,age:experience,g:been_allstar_before) %>%
  select(-c(mp,share))
```
...
```{r this_year_linear}
this_year_model<-linear_reg() %>% set_engine("lm") %>% set_mode("regression")
this_year_workflow<-workflow() %>% add_recipe(this_year_recipe) %>%
  add_model(this_year_model)
...
The most important variable is the difference in minutes per game. As a player gets more time on the court, there's more opportunity to rack up positive stats like rebounds and assists but also negative stats like personal fouls and turnovers. The new experience variable checks in as the 7th-most important variable in the linear variable, while the rate & total versions of win shares round out the top 10.
...
```{r this_year_knn}
this_year_model<-nearest_neighbor(neighbors = tune()) %>% 
  set_engine("kknn") %>% set_mode("regression")
this_year_workflow<-workflow() %>% add_recipe(this_year_recipe) %>%
  add_model(this_year_model)
tuning<-this_year_workflow %>% 
  tune_grid(resamples=this_year_cv,
            grid=expand.grid(neighbors=5:50),metrics=metric_set(rmse))
this_year_workflow <-this_year_workflow %>% 
  finalize_workflow(tuning %>%select_best(metric="rmse"))
knn_this_yr=fit(this_year_workflow,jumps_trainer)
tuning %>% select_best(metric="rmse")
```

Looks like the optimal amount of neighbors is 38.
```{r knn_1987,echo=FALSE}
model_results=bind_rows(model_results,eval_table(knn_this_yr,"KNN"))
model_results %>% knitr::kable()
```
The RMSE has improved and the probabilities have grown. The highest share in the linear model is less than half the 3rd highest share in the knn model. KNN predicts Dale Ellis as the winner. We've already discussed the shortcomings of modelling in regards to Jordan's 1987 season, as well as the merit of Thorpe's case.
