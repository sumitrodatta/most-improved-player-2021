...
all_stars=read_csv("Data/All-Star Selections.csv") %>% mutate(is_allstar=1) %>%
  ...
  # subtract 1 to remove current season
  mutate(num_allstars_before_seas=cumsum(is_allstar)-1) %>% ungroup() %>% 
  select(-c(team,replaced,hof))
final_data=left_join(stats_and_share,all_stars)
```
...
full_jumps_data<-final_data %>% 
  ...
  mutate_at(.vars=c(colnames(final_data[12:76])),
            .funs=funs(`diff`=.-lag(.,default=first(.)))) %>%
  #do not keep the raw stats, only differences
  select(seas_id:tm, g_diff:pts_per_game_diff,fg_per_100_poss_diff:vorp_diff,
         share,is_allstar,num_allstars_before_seas) %>% 
...
So our outcome column is one of the near-zero variance variables. The number of all-star selections also is near zero variance because anywhere from 20-30 players in a given season are named all-stars out of a total of 200-500 players. Interestingly, the 3-point percentage difference is also a near-zero variance column.
